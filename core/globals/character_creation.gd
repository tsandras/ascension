...
func set_step1_data(char_name: String, race: String, attr: Dictionary):
	"""Store data from character creation step 1"""
	character_name = char_name
	selected_race = race
	attributes = attr.duplicate()
	print("Stored step 1 data - Name: %s, Race: %s" % [character_name, selected_race])
...
func has_complete_data() -> bool:
	...
	return character_name != "" and selected_race != "" and attributes.size() > 0 
